              ...
              <el-row class="disFlexCenter">
                ...
                <div>
                  <el-col class="disFlexCenter">
                    ...
                    <div style="font-size: 0.85vw; height: 1vh; margin-left: 0.5vw">微信名称:</div>
                    <div>
                      <el-input v-model="listQuery.nick" placeholder="请输入微信名称" show-word-limit
                        style="width: 12vw; height: 1vh; margin-left: 0.5vw"></el-input>
                    </div>
                  </el-col>
                </div>
...
<!--        ">-->
        ...
        <template>
          <el-table ref="listdata" stripe v-loading="loading" element-loading-text="加载中"
            ...
            <el-table-column label="头像" min-width="100">
              <template slot-scope="scope">
                <el-image :src="scope.row.portrait" style="width: 5vw" :previewSrcList="srcFmtList"
                  v-if="scope.row.portrait != '' && scope.row.portrait != null && scope.row.portrait != undefined" />
              </template>
            </el-table-column>
            <el-table-column prop="phone" label="联系方式" min-width="200">
            </el-table-column>
            <el-table-column prop="ifjyzk" label="是否禁用" min-width="100" align="center">
              <template slot-scope="scope">
    ...
    <div class="dialog_out">
      <!-- 新增/修改用户 -->
      <el-dialog :visible.sync="dialogVisible1" width="60%" :before-close="handleClose1" top="5vh">
        <template slot="title">
          <div style="
              float: left;
              width: 0.5vw;
      ...
      <!-- 修改密码 -->
      <el-dialog :visible.sync="dialogVisible2" width="60%" :before-close="handleClose2" top="25vh" title="重置密码">
      ...
      <!-- 授权 -->
      <el-dialog :visible.sync="auth_show" width="60%" :before-close="handleClose3" top="25vh" title="重置密码">
        <div class="jsshu" style="
            font-size: 0.9vw;
            margin: 3vw auto 6vw;
            width: 70%;
  ...
  methods: {
    ...
    afterRead(file) {
      ...
      axios.post(this.url, params, cg).then((res) => {
        console.log('%c [ res ]-938-「e:/jiading/littlewish/littlewish-back/src/views/xtgl/yhgl」', 'font-size:13px; background:#34d5f0; color:#78ffff;', res)
    ...
    handleAvatarSuccess(res, file, fileList) {
      console.log('%c [ res ]-963-「e:/jiading/littlewish/littlewish-back/src/views/xtgl/yhgl」', 'font-size:13px; background:#ba4492; color:#fe88d6;', res)
      // 原图
